...
const getInstructorsm = (requester, page, limit) => {
    return new Promise((resolve, reject) => {
            ...
            client.query('SELECT COUNT(*) OVER() AS total_count, u.user_name, u.status, u.user_profile_photo, u.user_email, u.user_contact_num, u.user_dob, u.user_gender, bpd.batch_id, ARRAY_AGG(b.batch_name ORDER BY b.batch_id) AS batch_names FROM public.user_data u LEFT JOIN public.batch_people_data bpd ON u.user_email = bpd.user_id LEFT JOIN LATERAL unnest(bpd.batch_id) AS bid(batch_id) ON TRUE LEFT JOIN public.batch_data b ON b.batch_id = bid.batch_id WHERE u.user_role = ANY(ARRAY[$1]) GROUP BY u.user_name, u.user_email, u.user_contact_num, u.user_dob, u.user_gender, bpd.batch_id ORDER BY u.user_name LIMIT $2 OFFSET $3',['102', limit, offset], (err, result) => {
                if(err)
                {
                    return reject(err)
                }
