...
const supabase = createClient(
  process.env.SUPABASE_URL || 'https://rnrnzmqtvcyqhpakynls.supabase.co',
  process.env.SUPABASE_KEY // Make sure this is set in your environment
);
...
/**
 * Download file using direct URL (fallback)
 */
const downloadFromUrl = (url, volumeId) => {
  return new Promise((resolve, reject) => {
...
/**
 * Starts volume conversion and uploads to Supabase
 */
const startVolumeConversion = async (volumeId) => {
  console.log(`[${volumeId}] Starting volume conversion...`);
  ...
  try {
    ...
    // Verify Slicer and script exist
    if (!fs.existsSync(SLICER_PATH)) {
      console.error(`[${volumeId}] ERROR: 3D Slicer not found`);
      await updateConversionFailure(volumeId, "3D Slicer executable not found");
      return;
    }
    
    if (!fs.existsSync(SCRIPT_PATH)) {
      console.error(`[${volumeId}] ERROR: Python script not found`);
      await updateConversionFailure(volumeId, "Conversion script not found");
      return;
    }
    ...
    const slicerProcess = spawn(SLICER_PATH, [
      ...
      env: {
        ...process.env,
        SUPABASE_URL: process.env.SUPABASE_URL || 'https://rnrnzmqtvcyqhpakynls.supabase.co',
    ...
    slicerProcess.on("close", async (code) => {
      ...
      try {
        if (code === 0 && conversionResult && conversionResult.success) {
          // ✅ SUCCESS - File is now in Supabase
          console.log(`[${volumeId}] ✅ Conversion completed successfully`);
          console.log(`[${volumeId}] Supabase path: ${conversionResult.storage_path}`);
          console.log(`[${volumeId}] Public URL: ${conversionResult.public_url}`);
          console.log(`[${volumeId}] Size: ${conversionResult.file_size_bytes} bytes (${conversionResult.file_size_mb} MB)`);
          ...
          `, [
            volumeId, 
            conversionResult.storage_path, 
            conversionResult.file_size_bytes,
            conversionResult.file_size_kb,
            conversionResult.file_size_mb,
            conversionResult.public_url
          ]);
          ...
          await client.query(`
            ...
            WHERE volume_id = $1
          `, [volumeId, conversionResult.storage_path]);
...
const cleanupDownloadedFile = (filePath, volumeId) => {
  if (filePath && fs.existsSync(filePath)) {
    try {
      ...
      console.error(`[${volumeId}] Failed to cleanup file:`, err);
    }
  }
};
